...
class XTabularInput extends CWidget {
    ...
    // customization ..... for ck editor
    public $hasCkEditor = false;
    public $CKEDITORid = '';
    ...
    protected function registerClientScript() {
        ...
        $cs->registerScriptFile($assets . '/jquery.calculation.min.js', CClientScript::POS_END);
        $cs->registerCssFile($assets . '/tabinput.css', CClientScript::POS_END);
	...
	$("#{$this->id} .{$this->addCssClass}").click(function(event){
		...
		$.ajax({
			success: function(html){
				...
				try{
					CKEDITOR.replace( "PackageTranslate_"+index+"_description" );
				} catch (e){}
			},
			type: 'get',
			url: this.href,
	...
	$(document).on('click', '#{$this->id} .{$this->removeCssClass}', function(event) {
		event.preventDefault();
		$(this).parents("{$this->inputTagName}[class*='{$this->inputCssClass}']:first").remove();
    ...
    var isNotClicked = $("#{$this->id} .{$this->addCssClass}").parents(".{$this->containerCssClass}:first").children(".{$this->inputContainerCssClass}").html().length;

    if( isNotClicked == 0 )
	   $("#{$this->id} .{$this->addCssClass}").click();
SCRIPT;
    ...
    protected function renderContent() {
        foreach ($this->models as $index => $model) {
            ...
            if ($this->hasCkEditor) {
                $cs = Yii::app()->clientScript;
                $script = str_replace("###ID###", $index, "CKEDITOR.replace( " . $this->CKEDITORid . " );");
                $cs->registerScript(__CLASS__ . '#' . $this->id . '_' . $index, $script, CClientScript::POS_READY);
            }
        }
    }
